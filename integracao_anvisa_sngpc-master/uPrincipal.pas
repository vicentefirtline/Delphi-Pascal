...
uses
  ...
  system.types,System.Zip,System.Math,IdHashMessageDigest, Vcl.Mask;
type
...
begin
  ...
  try
    ...
    ZipFile.Open(ExtractFilePath('integraSngpc.exe')+'\sngpc.zip', zmWrite);
  ...
  hash:=MD5(ExtractFilePath('integraSngpc.exe')+'\sngpc.zip');

  retorno:=GetsngpcSoap.EnviaArquivoSNGPC(email,senha,FIleToByteArray(ExtractFilePath('integraSngpc.exe')+'\sngpc.zip'),hash);
